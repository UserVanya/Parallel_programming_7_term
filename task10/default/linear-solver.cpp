    ...
    : m_G (G), m_g (g)
{
  ...
  for (size_t i = 0; i < G.Size (); i++)
    m_info.at (0).push_back (
        std::make_tuple (i == 0 ? 0 : G.At (i, i - 1), G.At (i, i),
                         i == G.Size () - 1 ? 0 : G.At (i, i + 1), g.at (i)));
}

eqInfo
ReductionSolver::GetNewEquation (const eqInfo &eq1, const eqInfo &eq2,
                                 const eqInfo &eq3)
{
  ...
  auto alpha = -a2 / b1;
  auto betta = -c2 / b3;
  return std::make_tuple (alpha * a1, alpha * c1 + b2 + betta * a3, betta * c3,
                          alpha * f1 + f2 + betta * f3);
}
double
ReductionSolver::Solve (int i1, int i2, const eqInfo &eq)
{
  auto [a, b, c, f] = eq;
  auto result = f;
  if (i1 < 0)
...
std::vector<double>
ReductionSolver::SolveReduction ()
{
  for (size_t lvl = 1; lvl < m_lvlsNumber; ++lvl)
    {
      size_t infosNumber = m_n / pow (2, lvl) - 1;
      m_info.at (lvl).resize (infosNumber);
#pragma omp parallel for schedule(guided, 400) if (infosNumber > 1e3)
      for (size_t i = 0; i < infosNumber; i++)
        {
          m_info.at (lvl).at (i)
              = GetNewEquation (m_info.at (lvl - 1).at (2 * i),
      ...
      for (size_t i = 0; i < infosNumber; i++)
        {
          ...
          if (!m_done.at (middleIndex))
            {
              m_solution.at (middleIndex)
                  = Solve (middleIndex - step, middleIndex + step,
                           m_info.at (lvl).at (i));
