...
NonLinearSolver::l (size_t dotIndex, double y_l)
{
  ...
  std::cout << 1 - m_h * m_h * 1 / 12 * m_fy (m_dots.at (dotIndex - 1), y_l) << std::endl;
...
NonLinearSolver::GetFullg (const std::vector<double> &sol)
{
  ...
  for(int i = 1; i < m_dots.size() - 1; i++)
    gVector[i - 1] = g(i, i == 1 ? m_a : sol.at(i-2), sol.at(i-1), i == m_dots.size() - 2 ? m_b : sol.at(i));
  return gVector;
}
std::vector<double>
...
NonLinearSolver::Solve (double eps)
{
  ...
  do
    {
      BandMatrix G (GetFullL(internalSolutionCurrent), GetFullD(internalSolutionCurrent), GetFullU(internalSolutionCurrent));
  ...
  return internalSolutionCurrent;
}
