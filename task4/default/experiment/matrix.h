...
public:
   ...
   * @return double 
   */
  double At(size_t rowIndex, size_t colIndex) const;
  /** Returns reference for an element from position Matrix[rowIndex + 1][colIndex + 1] 
   * @brief 
   * 
   * @param rowIndex 
   * @param colIndex 
   * @return double& 
   */
  double& At(size_t rowIndex, size_t colIndex);
private:
  std::vector<double> m_data;
  size_t m_numRows, m_numCols;
};
double MultiplyVectors (const Matrix& lhs, size_t rowIndex, const Matrix& rhs, size_t colIndex);
